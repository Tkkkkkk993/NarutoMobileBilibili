extends Control
class_name CharIconManager

# ========== 导出变量 ==========
@export_file("*.json") var json_data_path: String = ""
## 头像场景（CharIconSlot 场景，留空用原生 TextureButton）
...
func _generate_icons():
	...
	for i in range(count_to_generate):
		...
		# ========== 核心：自动拼接路径 ==========
		# 结果：res://assets/entities/llx/portraits/icon_idle.png
		...
		icon_node.set_meta("suffix", data.get("suffix", ""))
		icon_node.set_meta("folder_id", folder_id)
		
		icon_node.scale.x = 0.7
		icon_node.scale.y = 0.7
...
func _check_button_states():
	if tabs.selected_tab_id == 0:
		visible = true
		for child in get_children():
			if child is CharIconSlot:
				if child.consume_press():
					_handle_icon_clicked(child)
	else:
		visible = false

# 具体的处理逻辑
func _handle_icon_clicked(clicked_slot: CharIconSlot):
	var num_id = clicked_slot.get_meta("num_id")
	
	if clicked_slot.num > 0:
		main.delete_char(clicked_slot.num - 1)
	else:
		var new_num = main.add_char(char_data_dict[str(int(num_id))])
		if new_num >= 0:
			clicked_slot.num = new_num + 1

func deselect(slot: CharIconSlot):
	slot.num = 0
